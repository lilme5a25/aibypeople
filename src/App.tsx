...
function App() {
  ...
  const services = [
    {
      ...
      description: 'Beat competitors by calling leads instantly. AI qualifies patients and automates follow-ups to fill your schedule no extra staff needed.'
    },
    {
      title: 'Roofing',
      ...
      text: 'IFBERWLUELRIBEFVCKUEVAUVECIFBERWLUELRIBEFVCKUEVAUVECIFBERWLUELRIBEFVCKUEVAUVECIFBERWLUELRIBEFVCKUEVAUVEC',
      description: 'Capture every storm lead 24/7, wake up dead customer lists for new estimates, and automate the entire insurance supplement process from adjuster packets to final payments.'
    },
    {
      title: 'HVAC',
      image: '/hvac-hero.png',
      ...
      text: 'IFBERWLUELRIBEFVCKUEVAUVECIFBERWLUELRIBEFVCKUEVAUVECIFBERWLUELRIBEFVCKUEVAUVECIFBERWLUELRIBEFVCKUEVAUVEC',
      description: 'Capture every HVAC lead 24/7. AI answers instantly, qualifies customers, and drives repeat business from your existing client base.'
    },
    {
      title: 'Plumbing',
      ...
      text: 'IFBERWLUELRIBEFVCKUEVAUVECIFBERWLUELRIBEFVCKUEVAUVECIFBERWLUELRIBEFVCKUEVAUVECIFBERWLUELRIBEFVCKUEVAUVEC',
      description: 'Capture every paid job. Stop missing calls. A complete "Done-For-You" AI system that handles all communication, booking, and billing.'
    }
  ];
  ...
  if (showClinicsPage) return <ClinicsPage onBack={() => setShowClinicsPage(false)} onGetInTouch={handleContactNavigation} />;
  if (showPlumbingPage) return <PlumbingPage onBack={() => setShowPlumbingPage(false)} onGetInTouch={handleContactNavigation} />;
  if (showHVACPage) return <HVACPage onBack={() => setShowHVACPage(false)} onGetInTouch={handleContactNavigation} />;
  if (showRoofingPage) return <RoofingPage onBack={() => setShowRoofingPage(false)} onGetInTouch={handleContactNavigation} />;
  if (showGetInTouchPage) return <GetInTouchPage onBack={() => setShowGetInTouchPage(false)} />;
    ...
    <>
      ...
      <div className={`menu-overlay ${menuOpen ? 'open' : ''}`}>
        <div className="menu-links">
          <button className="menu-link" onClick={() => handleMenuClick(setShowClinicsPage)}>Clinics</button>
          <button className="menu-link" onClick={() => handleMenuClick(setShowRoofingPage)}>Roofing</button>
          <button className="menu-link" onClick={() => handleMenuClick(setShowHVACPage)}>HVAC</button>
          <button className="menu-link" onClick={() => handleMenuClick(setShowPlumbingPage)}>Plumbing</button>
          <button className="menu-link" onClick={() => handleMenuClick(setShowGetInTouchPage)}>Get In Touch</button>
        </div>
      </div>

      {/* Navigation Bar */}
      <nav className={`main-nav ${navbarVisible ? 'visible' : ''}`} id="mainNav">
        {/* Left: Logo Icon */}
        ...
        {/* Right: Burger Menu */}
        <div className={`burger-menu ${menuOpen ? 'open' : ''}`} onClick={toggleMenu} style={{ cursor: 'pointer' }}>
          <span></span>
          <span></span>
          <span></span>
        </div>
      ...
      {!hideLogoElements && (
        <>
          ...
          <div className={`image-container ${isHyperSpin ? 'hyper-spin' : ''}`} id="logoContainer">
            {/* Simple SVG */}
            <svg
            ...
            >
              <path d="M419 195L209.5 0L0 195L209.5 410.5L419 195Z" fill="#F4F4F0" />
            </svg>
            <img
              src="/image 2.png"
              alt="Base Image"
      ...
      <div className={`image-grid ${gridVisible ? 'active' : ''}`} id="imageGrid">
        {services.map((service, index) => (
          <div
            key={index}
            ref={(el) => { cardRefs.current[index] = el; }}
            className={`grid-column ${imagesDropping ? 'fall' : ''} ${activeIndex === index ? 'active-card' : ''}`}
            onClick={() => {
          ...
          >
            ...
            <img src={service.image} alt={service.title} className="desktop-only-img" />
            ...
            <div className="card-overlay">
              <div className="mobile-content-wrapper">
                ...
                <div className={`mobile-expanded-details ${activeIndex === index ? 'expanded' : ''}`}>

                  {/* Tags */}
                  <div className="mobile-tags">
                    {service.buttons.map((btn, btnIndex) => (
                      <span key={btnIndex} className="mobile-tag-pill">{btn}</span>
                    ))}
                  </div>
